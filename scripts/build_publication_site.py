...
def sitemap(pubs):
    ...
    for path in ('ja/','en/','ja/research/','en/research/','ja/methods/','en/methods/','ja/about/','en/about/','ja/editorial-policy/','en/editorial-policy/','ja/corrections/','en/corrections/'):
...
def main():
    ...
    pubs=load('publishing/publications.json')['publications']; figs=load('publishing/figures.json')['figures']; validate(pubs,figs); fmap=figure_map(figs)
    for lang in ('ja','en'):
    ...
    (SITE/'sitemap.xml').write_text(sitemap(pubs),encoding='utf-8')
    ...
    print(f'Built {len(pubs)} editions and {len(figs)} figures; preview={args.preview}')
