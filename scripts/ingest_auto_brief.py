...
def main():
    ...
    if any(x['publication_id']==brief['publication_id'] and x['language']==brief['language'] for x in pubs):
        raise SystemExit('duplicate publication_id/language')
    if any(x.get('source_observation_id')==brief['source_observation_id'] for x in pubs):
        raise SystemExit('duplicate source_observation_id')
    ...
    entry={
      'publication_id':brief['publication_id'],'language':lang,'edition':'automatic-market-watch',
      'slug':slug,'title':title,'summary':brief['summary'],'content_type':'market_watch',
      'content_type_label':labels['watch'],'themes':[],'series':brief['series_id'],
      'published_at':published_at,'updated_at':published_at,'as_of':brief['as_of'],
      'status':'published','article_path':f'site/{lang}/research/{slug}/index.html',
      'url':f'{BASE}/{lang}/research/{slug}/','hero_figure':None,'related_publications':[],
      'homepage':{'eligible':True,'priority':20,'lead_candidate':False},
      'source_observation_id':brief['source_observation_id'],'auto_publish_profile':brief['auto_publish_profile']
    }
    pubs.append(entry); pubsdoc['updated_at_jst']=datetime.now().astimezone().strftime('%Y-%m-%d %H:%M JST')
    PUBS.write_text(json.dumps(pubsdoc,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')

    RECEIPTS.mkdir(parents=True,exist_ok=True)
    receipt={
      'schema_version':'signa-auto-publication-receipt-v0.1','publication_id':brief['publication_id'],
      'series_id':brief['series_id'],'source_observation_id':brief['source_observation_id'],
      'source_repo':args.source_repo,'source_sha':args.source_sha,'source_path':args.source_path,
      'validation_profile':brief['auto_publish_profile'],'shadow':bool(args.shadow),
      'production_url':entry['url'] if not args.shadow else None
    }
